...
3. Before changing Git state, reject an active merge, rebase, cherry-pick, or revert. Save the current `HEAD` under `refs/pi-refactor/backups/<timestamp>` and stash tracked plus untracked changes with a `pi-refactor backup` message. Record both recovery identifiers.
