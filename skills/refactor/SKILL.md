...
3. Reject active merge, rebase, cherry-pick, or revert operations. Save the current `HEAD` under `refs/agent-refactor/backups/<timestamp>`, then stash tracked and untracked changes with an `agent-refactor backup` message.
